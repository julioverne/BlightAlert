...
- (void)setBacklightLevel:(float)val
{
	BKSDisplayBrightnessSet(val+0.001f, 1);
...
- (void)startAlert
{
	...
	oldBlacklightLevel = BKSDisplayBrightnessGetCurrent();
...
{
	[[BLightAlertActivator sharedInstance] startAlert];
}
...
static void LoadPreferences()
{
	//@autorelease {
		...
		if([BLightAlertActivator sharedInstanceExist]) {
			[[BLightAlertActivator sharedInstance] stopAlert];
		}
	//}
}
